    ...
    render = () => {
        return (
            <div className="FileUploadForm">
                ...
                <button 
                    type="button" 
                    className="border border-grey-light button-bg p-5 hover-transition focus:outline-none rounded text-white verify-button" 
                    onClick={this.uploadFile}
                >Upload</button>
                <hr></hr>
                {this.state.status && <h4>{this.state.status}</h4>}
            </div>
        );
    }
}
