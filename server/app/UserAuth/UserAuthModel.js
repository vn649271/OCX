...
function UserAuth() {
    ...
    this.getById = async function(userId) {
        const snapshot = await db.collection(table_name).doc(userId).get();
        if (snapshot.empty) {
    ...
    this.all = async () => {
        let retArray = [];
        const userAuthRef = db.collection(table_name);
    ...
    this.create = async function (jsonUser) {
        ...
        const res = await db.collection(table_name).add(jsonUser);
        if (res !== null) {
            return res.id;
        }
        return null;
    ...
    this.setPinCode = async function (userId, pinCode) {
        const userRef = db.collection(table_name).doc(userId);
        let now = new Date();
