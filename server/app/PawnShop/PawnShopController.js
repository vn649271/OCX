...
class PawnShopController {
    ...
    getSubmitFee = async(req, resp, next) => {
        ...
        try {
            ...
            let ret = await pawnItemService.getSubmitFee(accountInfo);
            return resp.json(ret);
        } catch (error) {
    ...
    getWeeklyFee = async(req, resp, next) => {
        var userToken = req.body ? req.body.userToken ? req.body.userToken : null : null;
        if (userToken === null) {
            return resp.json({ error: -1, data: "Invalid request" });
        }
        try {
            ...
            let ret = await pawnItemService.getWeeklyFee(accountInfo, req.body.basePrice);
            return resp.json(ret);
        } catch (error) {
            let errorMessage = error.message.replace("Returned error: ", "");
            return resp.json({ error: -100, data: errorMessage });
    ...
    getPnftFee = async(req, resp, next) => {
        var userToken = req.body ? req.body.userToken ? req.body.userToken : null : null;
        if (userToken === null) {
            return resp.json({ error: -1, data: "Invalid request" });
        }
        try {
            ...
            let ret = await pawnItemService.getPnftFee(accountInfo);
            return resp.json(ret);
        } catch (error) {
            let errorMessage = error.message.replace("Returned error: ", "");
            return resp.json({ error: -100, data: errorMessage });
