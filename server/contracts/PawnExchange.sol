...
contract PawnExchange {
    ...
    function setPnftAddress(address payable _pnftAddress) public {
        require(creator == msg.sender, "PawnExchange.setPNFTAddress(): Caller for  must be creator");
        require(_pnftAddress != address(0), "PawnExchange.setPNFTAddress(): Invalid parameter");
        pnftAddress = _pnftAddress;
    }

    function setOcatAddress(address payable _ocatAddress) public {
        require(creator == msg.sender, "PawnExchange.setOcatAddress(): Caller for  must be creator");
        require(_ocatAddress != address(0), "PawnExchange.setOcatAddress(): Invalid parameter");
        ocatAddress = _ocatAddress;
    }

    function exchangeToOcat(uint256 nftID) public {
        require(msg.sender != address(this), "PawnExchange.exchangeToOcat(): Error: Caller couldn't be same to this address");
        require(msg.sender != address(0), "PawnExchange.exchangeToOcat(): Invalid caller");
        require(nftID > 0, "PawnExchange.exchangeToOcat(): Invalid NFT ID");
    ...
    function exchangeFromOcat(uint256 nftID) public {
        require(msg.sender != address(this), "PawnExchange.exchangeFromOcat(): Error: Caller couldn't be same to this address");
        require(msg.sender != address(0), "PawnExchange.exchangeFromOcat(): Invalid caller");
        require(nftID > 0, "PawnExchange.exchangeFromOcat(): Invalid NFT ID");
        uint256 ocatBalance = IERC20(ocatAddress).balanceOf(msg.sender);
        // Get price for the NFT
        (,,,,,,uint256 price,,) = PawnNFTs(payable(address(pnftAddress))).allPawnNFTs(nftID);
        // uint256 price = nftItem.price;
        require(ocatBalance >= price, "PawnExchange.exchangeFromOcat(): Insufficient balance of OCAT in the account");
        // safeTransferFrom: send NFT from caller to the address
